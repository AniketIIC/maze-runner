...
def tranformTheReferenceImage(image_2_transform):
    img = cv2.imread(image_2_transform)
    ...
    if maxWidth > 0 and maxHeight > 0:
        ...
        plt.show()
        cv2.imwrite(tranformed_img_name, cv2.cvtColor(out, cv2.COLOR_BGR2GRAY))
    else:
...
def liveTrack():
    ...
    while cap.isOpened():
        ...
        frame_img = tranformImageLiveTrack(frame, (0,0))
        ...
        if ret:
            ...
            frame_diff_list = []
            print(frame_img.shape, background.shape)
            # background_diff = cv2.resize(background, (frame.shape[1], frame.shape[0]))
            # cv2.imwrite("test.jpg",frame_diff)
            frame_diff = cv2.absdiff(frame_img, background)
            cv2.imshow("hfghjgj" ,frame_diff)
            ret, thres = cv2.threshold(frame_diff, THRESHOLD_VALUE, 255, cv2.THRESH_BINARY)
            dilate_frame = cv2.dilate(thres, None, iterations=2)
            frame_diff_list.append(dilate_frame)
            # consecutive_frame = args['consecutive_frames']
            print(len(frame_diff_list))
...
tranformTheReferenceImage(image_name)
