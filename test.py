...
maxWidth = int(ceil(max(width_AD, width_BC)))
...
maxHeight = int(ceil(max(height_AB, height_CD)))
...
if maxWidth > 0 and maxHeight > 0:
    ...
    #flipped_out = cv2.flip(out, 1)  # 1 for horizontal flip, 0 for vertical flip, -1 for both

    # Display the flipped output image
    plt.figure()
    plt.imshow(out)
    plt.title("Flipped Perspective Transformed Image")
    plt.show()
else:
